# 👋 Hi, I'm Kabiraj Kafle

🚀 SEO Analyst | Digital Marketer | Growth-Focused Problem Solver  
📍 Bhaktapur, Nepal  
🌍 Open to Remote & International Opportunities  

📧 kabirajk333@gmail.com  
📞 +977-9860357981  

---

## 🚀 About Me

I’m an SEO and Digital Marketing professional focused on **driving organic growth, improving rankings, and delivering measurable results**.

I combine **technical SEO, content strategy, and data analysis** to help businesses grow their online presence. My experience working with international teams has strengthened my ability to adapt, communicate, and execute effectively.

---

## 💼 What I Do

- 🔍 SEO Strategy (On-page + Off-page)
- ⚙️ Technical SEO & Site Audits
- 📊 Data Analysis (Google Analytics & Search Console)
- 🧠 Keyword Research & Competitor Insights
- ✍️ Content Optimization
- 🔗 Link Building Campaigns

---

## 📊 SEO Projects & Case Studies

### 🔹 Project 1: Organic Traffic Growth (Zeptosky)

**Objective:** Increase organic traffic and keyword rankings  

**Actions Taken:**
- Performed detailed keyword research targeting low-competition keywords  
- Optimized on-page SEO (meta tags, headings, internal linking)  
- Fixed technical SEO issues (site speed, indexing, crawl errors)    

**Results:**
- 📈 Increased organic traffic by **40%+** in 3 months  
- 🔝 Ranked multiple keywords on **Google Page 1**  
- ⚡ Improved site performance and user engagement  

---

### 🔹 Project 2: Technical SEO Audit & Fix

**Objective:** Improve website health and crawlability  

**Actions Taken:**
- Conducted full audit using Screaming Frog  
- Fixed broken links, duplicate content, and indexing issues  
- Optimized XML sitemap and robots.txt  

**Results:**
- ✅ Reduced crawl errors significantly  
- 🚀 Improved indexing rate  
- 📊 Better search visibility  

---

### 🔹 Project 3: Content Optimization Strategy

**Objective:** Improve rankings through content  

**Actions Taken:**
- Updated existing blog content with better keyword targeting  
- Improved readability and structure  
- Added internal linking strategy  

**Results:**
- 📈 Increased average ranking position  
- ⏱️ Improved user time-on-page  
- 📊 Boosted organic impressions  

---

### 🔹 Project 4: Local SEO Ranking Success (DPS Design)

**Website:** https://dpsdesign.com.au/  
**Focus:** Local SEO (Moorebank, Sydney)  

**Objective:**  
Rank the business on top of Google search results for local intent keywords, especially **“printing company in Moorebank”**.

https://github.com/kabiraj966/Portfolio/blob/main/Screenshot%202026-04-02%20145255.png?raw=true

**Actions Taken:**
- Optimized Google Business Profile (GMB) with accurate business details and keywords  
- Implemented local keyword targeting across website pages  
- Improved on-page SEO (meta titles, descriptions, headings with location intent)  
- Built local citations and ensured NAP consistency (Name, Address, Phone)  
- Created location-focused content to strengthen local relevance  
- Acquired quality backlinks to improve domain authority  

**Results:**
- 🥇 Ranked **#1 on Google** for the keyword *“printing company in Moorebank”*  
- 📍 Significantly improved visibility in local search results  
- 📈 Increased local inquiries and customer engagement  
- ⭐ Strengthened overall Google Business Profile performance  

---

## 🛠️ Tools & Technologies

- Google Analytics  
- Google Search Console  
- Screaming Frog  
- Ahrefs / SEMrush  
- Canva  
- Excel / Google Sheets  
- JIRA & Confluence  

---

## 💼 Experience

### **SEO Analyst**  
**Zeptosky (Australia)**  
📅 May 2024 – Jan 2026  

- Improved rankings using targeted SEO strategies  
- Conducted keyword & competitor analysis  
- Built backlinks and optimized content  
- Worked with developers on technical SEO  
- Delivered performance reports  

---

### **Digital Marketing Intern**  
Weblink Nepal IT Institute Pvt. Ltd.  
📅 Feb 2024 – May 2024  

- Managed social media platforms  
- Assisted in campaign execution  
- Supported SEO and traffic growth  

---

### **Web Development Intern**  
Six Sigma Inc. Pvt. Ltd.  
📅 Feb 2023 – May 2023  

- Built responsive websites  
- Collaborated on development projects  

---

## 🎓 Education

🎓 Bachelor in Information Management (BIM)  
NIST College, Nepal  
📅 2024 | CGPA: 3.33  

🎓 +2 (Computer Science)  
Chanakya College  
📅 2018 | GPA: 2.77  

---

## 📜 Certifications

- HubSpot SEO Certification
- Digital Marketing Certification 

---

## 🌍 Websites I've Worked On

Here are some of the websites I’ve contributed to through SEO, digital marketing, or web-related work:

- 🔗 https://zeptosky.com.au/  
- 🔗 https://dpsdesign.com.au/  
- 🔗 https://stoneybark.com.au/  
- 🔗 https://a1groupservices.com.au/  
- 🔗 https://www.trekkingadvisor.com/  
- 🔗 https://www.namastecargonepal.com/
- 🔗 https://www.everest-trekking.com/

> *Worked on improving SEO performance, optimizing content, fixing technical issues, and enhancing overall digital visibility across these platforms.*

## 🌐 Let's Connect

If you're looking for someone who can **grow your traffic, improve rankings, and deliver real SEO results**, let's connect.

📧 Mail: kabirajk333@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/kabiraj-kafle-977ab9159/

---

⭐ *Always learning. Always optimizing. Always growing.*
